...
local function FormatWeaponAttachments(itemdata)
    ...
    local WeaponAttachments = exports['kg-weapons']:getConfigWeaponAttachments()
        ...
        if componentHash then
            for _, attachmentData in pairs(itemdata.info.attachments) do
                if attachmentData.component == componentHash then
                    local label = KGCore.Shared.Items[attachmentType] and KGCore.Shared.Items[attachmentType].label or 'Unknown'
...
RegisterNetEvent('kg-inventory:client:requiredItems', function(items, bool)
    local itemTable = {}
    if bool then
        for k in pairs(items) do
            itemTable[#itemTable + 1] = {
                item = items[k].name,
                label = KGCore.Shared.Items[items[k].name]['label'],
                image = items[k].image,
            }
        end
    end
...
RegisterNetEvent('kg-inventory:client:hotbar', function(items)
...
RegisterNetEvent('kg-inventory:client:closeInv', function()
    SendNUIMessage({
        action = 'close',
    })
end)

RegisterNetEvent('kg-inventory:client:updateInventory', function()
    SendNUIMessage({
        action = 'update',
...
RegisterNetEvent('kg-inventory:client:ItemBox', function(itemData, type, amount)
    SendNUIMessage({
        action = 'itemBox',
        item = itemData,
        type = type,
...
RegisterNetEvent('kg-inventory:server:RobPlayer', function(TargetId)
    SendNUIMessage({
        action = 'RobMoney',
        TargetId = TargetId,
    })
...
RegisterNetEvent('kg-inventory:client:openInventory', function(items, other)
...
RegisterNetEvent('kg-inventory:client:giveAnim', function()
...
RegisterNUICallback('AttemptPurchase', function(data, cb)
    KGCore.Functions.TriggerCallback('kg-inventory:server:attemptPurchase', function(canPurchase)
        cb(canPurchase)
    end, data)
end)
...
RegisterNUICallback('CloseInventory', function(data, cb)
    ...
    if data.name then
        ...
        TriggerServerEvent('kg-inventory:server:closeInventory', data.name)
    elseif CurrentDrop then
        TriggerServerEvent('kg-inventory:server:closeInventory', CurrentDrop)
        CurrentDrop = nil
    end
    cb('ok')
end)
...
RegisterNUICallback('UseItem', function(data, cb)
    TriggerServerEvent('kg-inventory:server:useItem', data.item)
    cb('ok')
end)

RegisterNUICallback('SetInventoryData', function(data, cb)
    TriggerServerEvent('kg-inventory:server:SetInventoryData', data.fromInventory, data.toInventory, data.fromSlot, data.toSlot, data.fromAmount, data.toAmount)
    cb('ok')
end)

RegisterNUICallback('GiveItem', function(data, cb)
    local player, distance = KGCore.Functions.GetClosestPlayer(GetEntityCoords(PlayerPedId()))
    if player ~= -1 and distance < 3 then
        local playerId = GetPlayerServerId(player)
        KGCore.Functions.TriggerCallback('kg-inventory:server:giveItem', function(success)
            cb(success)
        end, playerId, data.item.name, data.amount)
    else
        KGCore.Functions.Notify(Lang:t('notify.nonb'), 'error')
        cb(false)
    end
end)
...
RegisterNUICallback('GetWeaponData', function(cData, cb)
    local data = {
        WeaponData = KGCore.Shared.Items[cData.weapon],
...
RegisterNUICallback('RemoveAttachment', function(data, cb)
    ...
    local allAttachments = exports['kg-weapons']:getConfigWeaponAttachments()
    local Attachment = allAttachments[data.AttachmentData.attachment][WeaponData.name]
    local itemInfo = KGCore.Shared.Items[data.AttachmentData.attachment]
    KGCore.Functions.TriggerCallback('kg-weapons:server:RemoveAttachment', function(NewAttachments)
...
CreateThread(function()
    exports['kg-target']:AddTargetModel(Config.VendingObjects, {
        options = {
            {
                type = 'server',
                event = 'kg-inventory:server:openVending',
...
for i = 1, 5 do
    RegisterCommand('slot_' .. i, function()
        ...
        if itemData.type == "weapon" then
            if holdingDrop then
                return KGCore.Functions.Notify("Your already holding a bag, Go Drop it!", "error", 5500)
            end
        end
        TriggerServerEvent('kg-inventory:server:useItem', itemData)
    end, false)

...
local function SetupShopItems(shopItems)
    ...
    if shopItems and next(shopItems) then
        for _, item in pairs(shopItems) do
            local itemInfo = KGCore.Shared.Items[item.name:lower()]
            if itemInfo then
                items[slot] = {
                    name = itemInfo['name'],
...
function SetInventory(source, items)
    ...
    if not Player.Offline then
        ...
        TriggerEvent('kg-log:server:CreateLog', 'playerinventory', 'SetInventory', 'blue', logMessage)
    end
end
...
function UseItem(itemName, ...)
    local itemData = KGCore.Functions.CanUseItem(itemName)
...
function GetSlots(identifier)
    ...
    local player = KGCore.Functions.GetPlayer(identifier)
    if player then
...
function CanAddItem(source, item, amount)
    ...
    local itemData = KGCore.Shared.Items[item:lower()]
...
function ClearInventory(source, filterItems)
    ...
    if not player.Offline then
        ...
        TriggerEvent('kg-log:server:CreateLog', 'playerinventory', 'ClearInventory', 'red', logMessage)
...
-- Finally, it triggers the 'kg-inventory:client:closeInv' event for the given source.
function CloseInventory(source, identifier)
    ...
    TriggerClientEvent('kg-inventory:client:closeInv', source)
end
...
function OpenInventoryById(source, targetId)
    ...
    local TargetPlayer = KGCore.Functions.GetPlayer(tonumber(targetId))
    if not KGPlayer or not TargetPlayer then return end
    if Player(targetId).state.inv_busy then CloseInventory(targetId) end
    local playerItems = KGPlayer.PlayerData.items
    ...
    TriggerClientEvent('kg-inventory:client:openInventory', source, playerItems, formattedInventory)
end
...
function OpenInventory(source, identifier, data)
    ...
    if not identifier then
        ...
        TriggerClientEvent('kg-inventory:client:openInventory', source, KGPlayer.PlayerData.items)
        return
    end
    ...
    if inventory and inventory.isOpen then
        TriggerClientEvent('KGCore:Notify', source, 'This inventory is currently in use', 'error')
        return
    end
    ...
    TriggerClientEvent('kg-inventory:client:openInventory', source, KGPlayer.PlayerData.items, formattedInventory)
end
...
function AddItem(identifier, item, amount, slot, info, reason)
    ...
    if not updated then
        ...
        if KGCore.Shared.SplitStr(item, '_')[1] == 'weapon' then
            if not inventory[slot].info.serie then
                inventory[slot].info.serie = tostring(KGCore.Shared.RandomInt(2) .. KGCore.Shared.RandomStr(3) .. KGCore.Shared.RandomInt(1) .. KGCore.Shared.RandomStr(2) .. KGCore.Shared.RandomInt(3) .. KGCore.Shared.RandomStr(4))
            end
    ...
    local resourceName = GetInvokingResource() or 'kg-inventory'
    TriggerEvent(
        'kg-log:server:CreateLog',
        'playerinventory',
        'Item Added',
        'green',
...
function RemoveItem(identifier, item, amount, slot, reason)
    if not KGCore.Shared.Items[item:lower()] then
